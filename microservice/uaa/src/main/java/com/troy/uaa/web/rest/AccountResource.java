...
//
    ...
    public ResponseEntity<UserDTO> getAccount() {
        ResponseEntity<UserDTO> re = Optional.ofNullable(userService.getUserWithAuthorities())
                .map(user -> new ResponseEntity<>(userMapper.userToDto(user), HttpStatus.OK))
                .orElse(new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR));
        UserDTO use = re.getBody();
        Set<String> s = roleRepository.getCodeByUser(SecurityUtils.getCurrentUserId());
        if (s.contains("ROLE_AD")){
            s.add("ROLE_ADMIN");
        }
        use.setAuthorities(s);
        return re;
    }
    /**
    ...
    @Timed
    public ResponseDTO<UserDTO> getAccount(HttpServletRequest request) {
        HttpSession session = request.getSession();
        RoleDTO roleDTO = (RoleDTO) session.getAttribute(Constants.SELECT_ROLE);
        ResponseEntity<UserDTO> re = Optional.ofNullable(userService.getUserWithAuthorities())
            .map(user ->new ResponseEntity<>(userMapper.userToDto(user), HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR));

        if (re.getStatusCodeValue()==200){
            UserDTO userDTO = re.getBody();
            userDTO.setRoleDTO(roleDTO);
            //获取租户信息
            ResponseDTO<TenantDTO> responseDTO = tenantService.queryTenant(userDTO.getTenantId());
            if (responseDTO.getCode().equals("200")){
                userDTO.setTenantDTO(responseDTO.getData());
            }
        }
        return new ResponseDTO<UserDTO>("200","操作成功",re.getBody());
    }
//    /**
