...
public class CheckCodeUtil {
    private final Logger log = LoggerFactory.getLogger(CheckCodeUtil.class);
    ...
    public void getRandcode(HttpServletRequest request, HttpServletResponse response, String key) {
        ...
        log.info("[验证码获取]*************" + randomString + "[SessionID]:" + request.getSession().getId());
        ...
        try {
            ...
            log.error("[CheckCodeUtil Class] getRandcode 生成验证码失败", e);
        } finally {
            try {
                response.getOutputStream().flush();
                response.getOutputStream().close();
            } catch (Exception e2) {
                log.error("[CheckCodeUtil Class] getRandcode 生成验证码失败", e2);
            }
        }
    }
    ...
    返回ByteArray数组
     */
    public byte[] getgetRandcodeByByteArray(HttpServletRequest request, HttpServletResponse response, String key) {
        // BufferedImage类是具有缓冲区的Image类,Image类是用于描述图像信息的类
        BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_BGR);
        Graphics g = image.getGraphics();// 产生Image对象的Graphics对象,改对象可以在图像上进行各种绘制操作
        g.fillRect(0, 0, width, height);
        ...
        try {
            ...
            log.error("[CheckCodeUtil Class] getgetRandcodeByByteArray 生成验证码：ToByteArray输出失败", e);
        }
        return null;
    }
}
