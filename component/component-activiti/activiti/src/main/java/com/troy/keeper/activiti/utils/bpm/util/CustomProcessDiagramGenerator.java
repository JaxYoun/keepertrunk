package com.troy.keeper.activiti.utils.bpm.util;

import com.troy.keeper.activiti.utils.bpm.graph.ActivitiHistoryGraphBuilder;
import com.troy.keeper.activiti.utils.bpm.graph.Edge;
...
public class CustomProcessDiagramGenerator {
    ...
    private static final Color RUNNING_COLOR = Color.RED;

    private static final Color HISTORY_COLOR = Color.decode("#337ab7");

    private static final Color SKIP_COLOR = Color.GRAY;

    private static final Stroke THICK_BORDER_STROKE = new BasicStroke(3.0f);
...
//        if (CollectionUtils.isEmpty(flowNodes) && CollectionUtils.isEmpty(bpmnModel.getPools())
//                && (nrOfLanes == 0)) {
//            // Nothing to show
//            theMinX = 0;
//            theMinY = 0;
        ...
        try {
            ...
            for (int i = 0; i < xPoints.length; i++) {
                ...
                if ((i > 0) && (i < (xPoints.length - 1))) {
                    ...
                    double lineLengthY = (double)(yPoints[i] - yPoints[i - 1]);

                    // pivot point of prev line
                    double lineLengthX = (double)(xPoints[i] - xPoints[i - 1]);
                    ...
                    lineLengthY = (double)(yPoints[i + 1] - yPoints[i]);
                    lineLengthX = (double)(xPoints[i + 1] - xPoints[i]);
