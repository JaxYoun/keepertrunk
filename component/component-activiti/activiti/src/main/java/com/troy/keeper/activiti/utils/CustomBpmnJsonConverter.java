...
public class CustomBpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
    protected static final Logger LOGGER = LoggerFactory.getLogger(CustomBpmnJsonConverter.class);
    ...
    public BpmnModel convertToBpmnModel(JsonNode modelNode) {
        ...
        Map<String, CustomBpmnJsonConverter.FlowWithContainer> allFlowMap = new HashMap<String, CustomBpmnJsonConverter.FlowWithContainer>();
                ...
                for (ExtensionElement orderElement : orderList) {
                    ...
                    if (StringUtils.isNotEmpty(flowValue)) {
                        if (allFlowMap.containsKey(flowValue)) {
                            CustomBpmnJsonConverter.FlowWithContainer flowWithContainer = allFlowMap.get(flowValue);
    ...
    private void postProcessElements(FlowElementsContainer parentContainer, Collection<FlowElement> flowElementList,
                                     Map<String, JsonNode> edgeMap, BpmnModel bpmnModel, Map<String, CustomBpmnJsonConverter.FlowWithContainer> allFlowMap, List<Gateway> gatewayWithOrderList) {
        ...
        for (FlowElement flowElement : flowElementList) {
            ...
            if (flowElement instanceof BoundaryEvent) {
                ...
                if (sourceFlowElement != null && sourceFlowElement instanceof FlowNode) {
                    ...
                    CustomBpmnJsonConverter.FlowWithContainer flowWithContainer = new CustomBpmnJsonConverter.FlowWithContainer(sequenceFlow, parentContainer);
    ...
    private void readEdgeDI(Map<String, JsonNode> edgeMap, Map<String, List<JsonNode>> sourceAndTargetMap, BpmnModel bpmnModel) {
        ...
        for (String edgeId : edgeMap.keySet()) {
            ...
            JsonNode flowName = edgeNode.get("properties").get("name");
            if (null != flowName && !"".equals(flowName)) {
                ...
                if(graphicInfoList.size() > 1) {
                    double x0 = graphicInfoList.get(0).getX();
                    double y0 = graphicInfoList.get(0).getY();
                    double x1 = graphicInfoList.get(1).getX();
                    double y1 = graphicInfoList.get(1).getY();
                    x = x0;
                    y = y0;
                    if(x1 < x0){
                        x = x0 -  flowName.asText().length() * charSize;
                    } else if(x1 >= x0){
                        x = x0 + offset;
                    }
                    if(y1 < y0){
                    ...
                    x = graphicInfoList.get(0).getX();
                    y = graphicInfoList.get(0).getY();
                }
                bpmnModel.addLabelGraphicInfo(
                        edgeId,
                        createGraphicInfo(x, y));
            }
        }
    }
